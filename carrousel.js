function autoplayCarousel() {
  ...
  const navigate = (arg) => {
    slideContainerEl.scrollLeft = getNewScrollPosition(arg); // Cambié little a scrollLeft
  };
    ...
    (entries, observer) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          // Remover la clase active del indicador anterior
          carouselEl
            .querySelector(".slide-indicator.active")
            .classList.remove("active");
          // Agregar la clase active al nuevo indicador
          carouselEl
